...
export function splitDataset(
  ...
  if (
    ...
    throw new Error(
      'O dataset precisa possuir pelo menos 2 exemplos.'
    );
  }
  ...
  if (
    ...
    throw new Error(
      'testRatio deve estar entre 0 e 1.'
    );
  }

  const indexed = examples.map(
    (example, index) => ({
      example,
      index,
    })
  ...
  const train =
    shuffled.filter(
      example => !testSet.has(example)
    );

  if (train.length === 0) {
    throw new Error(
      'A divisão gerou um conjunto de treinamento vazio.'
    );
  }
...
export function evaluateIntentClassifier(
  trainingExamples: readonly TrainingExample[],
  testExamples: readonly TrainingExample[]
): IntentEvaluationResult {
  if (
    trainingExamples.length === 0
  ) {
    throw new Error(
      'O conjunto de treinamento está vazio.'
    );
  }
