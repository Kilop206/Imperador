...
export interface NeuralMemorySearchOptions {
  minimumScore?: number;
  topK?: number;
}
...
export interface NeuralMemoryModelSnapshot {
 ...
 */
export class NeuralSemanticMemoryService {
  private wordModel: WordEmbeddingModel | null = null;
  private sentenceModel: SemanticSentenceModel | null = null;

  private memories: NeuralMemory[] = [];
  ...
  public setModels(
    ...
    if (!wordModel.isTrained()) {
      throw new Error(
        "WordEmbeddingModel precisa estar treinado.",
      );
    }

    if (!sentenceModel.isTrained()) {
      throw new Error(
        "SemanticSentenceModel precisa estar treinado.",
      );
    }
  ...
  public setModelsFromRegistry(
    ...
    if (!active) {
      throw new Error(
        "Nenhuma versão ativa no registry.",
      );
    }

    const restored = registry.restoreModel(
      active.version,
    );

    if (!restored) {
      throw new Error(
        "Não foi possível restaurar o modelo ativo do registry.",
      );
    }
   ...
   */
  public add(memory: NeuralMemory): void {
   ...
   */
  public addMany(memories: NeuralMemory[]): void {
    if (!Array.isArray(memories)) {
      throw new TypeError(
        "memories deve ser um array.",
      );
    }
  ...
  public search(
    text: string,
    options: NeuralMemorySearchOptions = {},
  ): NeuralMemoryResult[] {
    ...
    const results: NeuralMemoryResult[] = [];

    for (
      let index = 0;
      index < this.memories.length;
      index += 1
    ) {
      ...
      const score = SemanticSentenceModel.cosineSimilarity(
        queryEmbedding,
        embedding,
      );
   ...
   */
  public rebuildEmbeddings(): void {
    this.embeddings = this.memories.map(
      (memory) => this.encodeOrZero(memory.text),
    );
  }

  // ─── Acesso a dados ────────────────────────────────────────────────────────

  public getMemories(): NeuralMemory[] {
   ...
   */
  public exportSnapshot(): NeuralMemoryModelSnapshot {
    return {
  ...
  public importSnapshot(
    snapshot: NeuralMemoryModelSnapshot,
  ): this {
    if (
      !snapshot ||
      ...
      throw new TypeError(
        "Snapshot de memória neural inválido.",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Memórias e embeddings com tamanhos incompatíveis.",
      );
    }
    ...
    for (const embedding of snapshot.embeddings) {
      if (!Array.isArray(embedding)) {
        throw new TypeError(
          "Embedding inválido no snapshot.",
        );
      }
    }

    this.memories = snapshot.memories.map(
      (m) => this.normalizeMemory(m),
    );

    this.embeddings = snapshot.embeddings.map(
      (e) => [...e],
    );
  ...
  public compare(first: string, second: string): number {
    ...
    return this.sentenceModel!.similarity(
      this.wordModel!,
      first,
      second,
    );
  ...
  private encodeOrZero(text: string): number[] {
    if (!this.isReady()) {
      return [];
    }

    return this.sentenceModel!.encode(
      this.wordModel!,
      text,
    );
  }

  private validateMemory(memory: NeuralMemory): void {
    if (
      typeof memory?.id !== "string" ||
      memory.id.trim().length === 0
    ) {
      throw new TypeError(
        "Memória neural inválida: id ausente ou vazio.",
      );
    }
    ...
    if (
      ...
      throw new TypeError(
        "Memória neural inválida: text ausente ou vazio.",
      );
    }
  }
  ...
  private ensureReady(): void {
    if (!this.isReady()) {
      throw new Error(
        "Modelos não configurados. Chame setModels() antes de usar o serviço.",
      );
    }
  }
}
